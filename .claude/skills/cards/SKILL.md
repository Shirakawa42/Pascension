...
- **Everything is respondable (2026-07-08, reverses the short-lived mana-ability rule)**: EVERY play and tap ability uses the stack and opens a response window — including pure-AP cards like Move/Run/Sprint/Redbull. A future card may opt out via `.ManaAbility()` / `TapAbility(..., manaAbility: true)`, but then its rules text/description MUST contain the words "can't be responded to" (pinned by `RespondableInvariant_NoCardOptsOut_WithoutSayingSoInItsText`). No current card opts out.
