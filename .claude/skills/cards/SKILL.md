...
- **Mana abilities (2026-07-08 rules change)**: a card or tap ability whose ONLY effect is gaining AP **never uses the stack and cannot be responded to** (like MTG mana abilities). Flag with `.ManaAbility()` / `TapAbility(..., manaAbility: true)`. Current set (pinned by `ManaFlag_RegistryInvariant` test — update BOTH when it changes): cards `move`, `redbull`, `run`, `sprint`; taps `cloth_armor`, `tower_shield`, `dragonscale_armor`. Cards with ANY extra effect (e.g. `adrenaline_shot` +2 AP *draw 1*) still stack and are counterable.
- **Deck transparency**: every player's deck CONTENTS are public (browsable alphabetically — order stays hidden). Hands remain hidden, but note contents are inferable by tracking. Deliberate casual-friendly choice.
