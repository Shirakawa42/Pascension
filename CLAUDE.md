...
- **SoI UI (M6, verified live 2026-07-12)**: `Assets/Scripts/Game/Soi/` — `SoiGameScreen` builds ALL its UI at runtime in Bind (LobbyScreen pattern: nothing serialized, no Init-order gotchas) and re-renders every zone per snapshot (no drag/staging — SoI has no response windows, straight rebuild is correct). `SoiCardWidget` (code-built card visual, faction frames, cost/shield/defense badges, damage marks, hover→fixed top-left preview), `SoiDecisionModal` (generic list mode + "soi.split" stepper mode with ALL→X shortcuts; CONFIRM gated on [Min,Max]), `SoiBootstrap` (solo via ShardsModule/GameCatalog; networked binds SessionProvider like GameBootstrap). Scene `GameShards.unity` from `SceneConstruction.BuildShardsGameScene` (thin: camera/canvas/UiRoot + 2 components). Menu: SoI solo panel (character grid rebuilt per DLC — Rez appears with SoS; 3 DLC toggles; 1-3 bots). **GameScreenBase was deliberately NOT extracted** — Pascension's GameScreen is untouched (pixel-identical by construction); revisit only if a third game lands. MainMenu.ShowPanel must list EVERY panel (missing entries render two panels at once — bit us with the game-select overlay). Verified via MCP-over-HTTP (raw JSON-RPC on :8090, scratchpad mcp.py) driving REAL UI events: full turn cycle (play/buy via mercenary popup/focus/end turn/bot turn/redraw), ItH game (destiny row, 117-card deck with Corruption auto-removed sans RotF), split modal 3/2 across two bots, Pascension solo regression screenshot.
- **M7 art import**: `Pascension/Setup/Shards Art Import` window reads `Tools/soi_art_sources.json` ({id, source} url-or-path) → `Assets/Art/Shards/Cards/{id}.png` → rebuilds the shared CardArtIndex (SoI ids never collide with Pascension's; Rebuild + EnsureCardArtIndex both scan the Shards folder). **`Assets/Art/Shards/` is git-ignored — imported official art is PERSONAL USE ONLY, never distribute.** Fallback for missing ids: text-frame cards render fine, or generate original art via the ComfyUI window with the defs' ArtPrompt fields.
