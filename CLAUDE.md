...
Plan: `C:\Users\Shira\.claude\plans\this-is-an-empty-quirky-sunbeam.md` (approved; M1-M3 + M5-scaffold done). Verified SoI rules spec lives in the plan; card DB compilation in `Tools/ShardsData/` (rules-notes.md + cards.json — compiled from the official Stone Blade rules PDFs; personal fan project, functional paraphrases only, art imported for private use only).
...
- **Shards.Engine**: no stack — single-pending-input pump; end-turn = split decision ("soi.split", option id per damage point) -> per-defender shield reveal ("soi.shields", shields stay in hand, champion shields passive) -> eliminate/cleanup (fast-played mercs -> BOTTOM of center deck; redraw 5 with mid-draw reshuffle). Focus once/turn (exhaust character + 1 gem -> +1 mastery, cap 30, staggered start 0/1/2/3). Champions die to full defense in one action. Immediate row refill. AtMastery(n) checks at play time AFTER earlier effect lines (own gain counts). DLC flags: RotF (relics@M10 choose 1 of 2, once, free), SoS-competitive (Rez), ItH (destinies@M5 + Ingeminex). 12 structural tests green on a stub card set. **Pump gotcha (tested): never resume a decision-parked effect iterator — PumpEffects guards on pending decision.**
...
- **Remaining**: wire cards.json -> Shards.Content builders + per-card rules tests + TODO-VERIFY seams (4th faction name, cleanup hand-discard, champion-shield detail, monster disposal, destiny acquisition) · ShardsHeuristicBot · ShardsModule registration (+ lobby character catalog: Decima/Ko Syn Wu/Tetra/Volos + Rez w/ DLC2) · GameScreenBase extraction (screenshot-diff Pascension!) · GameShards.unity + Soi views (center row/player boards/mastery dial/champion rows/damage-split modal) · art import window (`Tools/soi_art_sources.json`, personal use, git-ignored) + Anima fallback · full battery (Pascension regression + SoI solo/Relay).
