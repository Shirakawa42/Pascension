...
- **Remaining**: M6 — GameScreenBase extraction (screenshot-diff Pascension!) + GameShards.unity + Soi views (center row/player boards/mastery dial/champion rows/damage-split modal/destiny row/Ingeminex space) + flip `ShardsModule.Playable` · M7 — art import window (`Tools/soi_art_sources.json`, personal use, git-ignored) + Anima fallback · M8 — full battery (Pascension regression + SoI solo/Relay).
