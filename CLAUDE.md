...
- **Verified live (host side)**: NGO StartHost → lobby scene sync → lobby replication/bots → HostStartGame → networked GameHost + bridge + full turns; pause overlay + kick→bot + lockout + resume (white-box injected disconnect); UGS error path without cloud link. **Still needs a real 2-client MPPM battery over Relay once the project is linked (M0)** — join by code, per-client masked state, rejoin/resync, client-side pause overlay, host-loss overlay.
